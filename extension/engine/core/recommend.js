...
const usableDisplayMarket=player=>Number.isFinite(Number(player?.adp))&&Number(player.adp)>0&&Number(player.adp)<500;
function displayedAvailability({player,targetPick,currentPick,waitingForUserPick,state}){
  const calculate=waitingForUserPick?availabilityProbability:availabilityAfterSelection;
  if(!targetPick)return{probability:1,confidence:usableDisplayMarket(player)?"market":"low"};
  if(!usableDisplayMarket(player))return{probability:.5,confidence:"low"};
  // Tight-end market timing remains excluded from decision scoring until its
  // calibration gate passes, but a real provider ADP is still valid for display.
  const displayPlayer=String(player?.position||"").toUpperCase()==="TE"?{...player,position:"TE_DISPLAY_ONLY"}:player;
  return{probability:calculate(displayPlayer,targetPick,currentPick,{season:state.projectionSeason,settings:state.settings}),confidence:"market"};
}
...
function rankedRecommendationCandidates({ state, userSlot, strategy = "balanced", sourceProfile="projectionLed", customWeights, positions, limit = 8, includeAllEligible = false }) {
  ...
  const baseRanked=valued.map(row => {let {projectionValue,ceilingValue,floorValue}=row;const {player,replacementPoints}=row;
    const decisionAvailability = availabilityTargetPick ? (waitingForUserPick?availabilityProbability:availabilityAfterSelection)(player,availabilityTargetPick,currentPick,{season:state.projectionSeason,settings:state.settings}) : 1,displayAvailability=displayedAvailability({player,targetPick:availabilityTargetPick,currentPick,waitingForUserPick,state});
...
export function buildCandidateBoard(input){
  ...
  for(const player of state.players||[]){
    ...
    const displayAvailability=displayedAvailability({player,targetPick:availabilityTargetPick,currentPick,waitingForUserPick,state}),marketFields={nextPickAvailability:displayAvailability.probability,availabilityConfidence:displayAvailability.confidence,nextUserPick:userNextPick,availabilityTargetPick,waitingForUserPick};
