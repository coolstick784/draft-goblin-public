...
async function enrichState(state){
  ...
  const[baseline,draftGoblinFeed,fantasyPros,sleeper]=await Promise.all([baselinePromise,api(`/v1/projections/draftgoblin?season=${season}&scoring=${scoring}`).catch(()=>({available:false,players:[]})),fantasyProsPromises.get(fpKey).promise,state.platform==="espn"?sleeperProjectionPromises.get(fpKey):Promise.resolve({available:false,players:[]})]);
...
async function refresh(sequence=refreshSequence){
  ...
  try{
    ...
    if(stableData&&(stableAvailable.recommendations||[]).length>=stableMinimum){
      ...
      data=reusablePreliminary?filterRecommendationsByPositions(removeUnavailableRecommendationsBase(reusablePreliminary,state),selectedPositions()):await localApi("/v1/quick-evaluate",{method:"POST",body:JSON.stringify(evaluationPayload)});
      if(sequence!==refreshSequence)return;
...
chrome.storage.onChanged.addListener((changes,area)=>{
  if(area==="local"&&changes.projectionFeedCacheV1){fantasyProsPromises.clear();sleeperProjectionPromises.clear();resetDraftPresentation();scheduleRefresh();return}
  ...
  chrome.storage.session.get("activeDraftTab").then(({activeDraftTab})=>{
    ...
    if(pickOrClockChanged&&!windowChanged)showLiveDraftPosition(livePresentationState(latest));
