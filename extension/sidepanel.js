...
const tutorialSteps=()=>[
  {target:$("setupCoach"),title:"Open your draft",text:"Choose ESPN, Sleeper, or Yahoo, then enter a snake draft. Draft Goblin will connect automatically."},
...
const providerProjectionRows=player=>{
  ...
  add(platformNameKey,platformNameKey==="espn"?"ESPN":platformNameKey==="yahoo"?"Yahoo":"Sleeper",platform?.platformProjection);
...
const setTextIfChanged=(node,value)=>{const requested=String(value),next=node.id==="status"&&requested==="Recommendations unavailable"&&String(currentLiveState?.platform||"").toLowerCase()==="yahoo"?"Calculating decision context":requested;if(node.textContent!==next)node.textContent=next;if(node.id==="status"&&node.title!==next)node.title=next};
...
const draftCompletionState=state=>{const teams=Number(state?.settings?.teams),rounds=Number(state?.settings?.rounds),userSlot=Number(state?.userSlot),picks=Array.isArray(state?.picks)?state.picks:[],expectedPicks=teams*rounds,settingsReady=Number.isInteger(teams)&&teams>=2&&Number.isInteger(rounds)&&rounds>=1,userPicks=Number.isInteger(userSlot)&&userSlot>=1?picks.filter(pick=>Number(pick.slot)===userSlot).length:0;return{expectedPicks:settingsReady?expectedPicks:0,userPicks,draftComplete:settingsReady&&picks.length>=expectedPicks,userRosterComplete:settingsReady&&Number.isInteger(userSlot)&&userSlot>=1&&userPicks>=rounds}};
function showLiveDraftPosition(state){$("newDraftReady").hidden=true;const teams=Number(state.settings?.teams),rounds=Number(state.settings?.rounds),userSlot=Number(state.userSlot),currentPick=detectedCurrentPick(state),contextKey=userPickContext(state),completion=draftCompletionState(state);if(!Number.isInteger(teams)||teams<2||!Number.isInteger(rounds)||rounds<1||!Number.isInteger(userSlot)||userSlot<1)return;if(completion.draftComplete||completion.userRosterComplete||currentPick>teams*rounds){setTextIfChanged($("decisionAuthority"),"Final roster report");const prep=$("draftPrep");prep.hidden=false;$("prepTitle").textContent=completion.draftComplete?"Draft complete":"Your roster is complete";$("prepHelp").textContent=completion.draftComplete?"Building your final roster report.":"Waiting for the final league picks, then your draft report will be ready.";document.querySelector(".section-title h2").textContent="Draft finishing";return}const onClock=snakeSlot(currentPick,teams)===userSlot,nextUserPick=nextPickForSlot(currentPick-1,userSlot,teams,rounds);if(!onClock&&!Number.isInteger(nextUserPick)){setTextIfChanged($("decisionAuthority"),"Final roster report");return}setTextIfChanged($("decisionAuthority"),"You make the pick");const prep=$("draftPrep"),samePresentation=renderedContextKey===contextKey&&Boolean(cards.querySelector(".card:not(.skeleton)")),resultVisible=samePresentation&&titleOddsReady(renderedTitleEvidenceData);prep.hidden=false;$("prepTitle").textContent=onClock?`You are on the clock · Pick ${currentPick}`:`Preparing for your pick ${nextUserPick}`;if(!resultVisible){$("prepHelp").textContent=onClock?"Calculating your decision brief.":`Calculating options for pick ${nextUserPick}.`;if(!samePresentation)document.querySelector(".section-title h2").textContent="Comparing your options";if(exactFailureContextKey===contextKey)showExactOddsFailure(exactFailureMessage,contextKey);else if(consecutiveEvaluationFailures<3||cards.querySelector(".card:not(.skeleton)"))setTextIfChanged($("status"),"Calculating decision context");return}$("prepHelp").textContent=onClock?"Your decision brief is ready.":`Your options for pick ${nextUserPick} are ready.`;document.querySelector(".section-title h2").textContent=onClock?"Your options at this pick":"Options for your next pick"}
...
function showEspnPickSync(state){const platform=platformName(state?.platform);resetStableEvaluation();showLiveDraftPosition(state);setTextIfChanged($("status"),`Syncing · ${platform}`);const currentPick=detectedCurrentPick(state);$("prepHelp").textContent=`${platform} has moved to pick ${currentPick}. Syncing the latest completed pick before calculating recommendations…`;$("chance").textContent="—";$("range").textContent=`Waiting for ${platform} pick history to catch up.`;$("freshness").textContent=`Syncing ${platform}…`;cards.removeAttribute("aria-busy");cards.innerHTML=`<div class="empty">The ${platform} clock is ahead of its pick history. Recommendations will resume automatically as soon as the missing pick appears.</div>`}
...
function renderPlayerBoard(){
  if(!boardRows)return;
  const platformKey=String(currentLiveState?.platform||"espn").toLowerCase(),platformLabel=platformKey==="sleeper"?"Sleeper":platformKey==="yahoo"?"Yahoo":"ESPN",platformSort=$("boardPlatformSort");
...
function showDraftHandled(){clearDraftReport();$("newDraftReady").hidden=false;$("draftPrep").hidden=true;$("status").textContent="Waiting for a draft";$("chance").textContent="—";$("range").textContent="Open an ESPN, Sleeper, or Yahoo snake draft.";$("freshness").textContent="";cards.innerHTML='<div class="empty">Draft complete. Start a new draft when you are ready.</div>'}
async function startNewDraft(){resetDraftPresentation();draftGoblinPromises.clear();fantasyProsPromises.clear();sleeperProjectionPromises.clear();await chrome.runtime.sendMessage({type:"START_NEW_DRAFT"}).catch(()=>null);$("newDraftReady").hidden=true;$("draftPrep").hidden=true;$("status").textContent="Waiting for a new draft";$("chance").textContent="—";$("range").textContent="Open an ESPN, Sleeper, or Yahoo snake draft.";$("freshness").textContent="";cards.innerHTML='<div class="empty">Waiting for a new draftboard.</div>'}
const platformName=value=>String(value||"").toLowerCase()==="espn"?"ESPN":String(value||"").toLowerCase()==="sleeper"?"Sleeper":String(value||"").toLowerCase()==="yahoo"?"Yahoo":"draft";
...
async function findState(sequence=refreshSequence){
  ...
  if(!state){
    ...
    $("chance").textContent="—";
    $("range").textContent="Open an ESPN, Sleeper, or Yahoo snake draft.";
...
async function enrichState(state){
  ...
  const platformName=state.platform==="espn"?"ESPN":state.platform==="yahoo"?"Yahoo":"Sleeper",platformOption=$("projectionPlatformOption");if(platformOption)platformOption.textContent=`Current draft site (${platformName})`;
...
async function refresh(sequence=refreshSequence){
  ...
  try{
    ...
    const completion=draftCompletionState(state),expectedPicks=completion.expectedPicks;
    const draftComplete=completion.draftComplete;
    ...
    if(platformMarkedComplete&&!draftComplete){
      ...
      $("prepHelp").textContent=`${platformName(state.platform)} has marked the draft complete. Waiting for ${expectedPicks-state.picks.length} final pick${expectedPicks-state.picks.length===1?"":"s"} before building your report.`;
      ...
      $("freshness").title=`The final ${platformName(state.platform)} pick history is still propagating.`;
      cards.removeAttribute("aria-busy");
    ...
    $("reportReady").hidden=true;
    const userRosterComplete=completion.userRosterComplete;
...
chrome.storage.onChanged.addListener((changes,area)=>{
  ...
  chrome.storage.session.get("activeDraftTab").then(({activeDraftTab})=>{
    ...
    const completion=draftCompletionState(latest),terminalStatusOnly=draftPhaseChanged&&!newDraft&&!picksChanged&&completion.draftComplete;
    if(newDraft||picksChanged||draftPhaseChanged&&!terminalStatusOnly){
...
const superviseAndRefresh=()=>{superviseDraftConnection();if(exactFailureContextKey&&exactContextStillCurrent(exactFailureContextKey)){if(exactAutoRetryContextKey!==exactFailureContextKey){exactAutoRetryContextKey=exactFailureContextKey;exactAutoRetryCount=0;exactAutoRetryAfter=0}if(exactAutoRetryCount<3&&Date.now()>=exactAutoRetryAfter){exactAutoRetryCount++;exactAutoRetryAfter=Date.now()+1000*2**exactAutoRetryCount;retryExactOdds();return}}if(!stableRefinedResult&&!exactEvaluationPromise&&!pendingEvaluation(lastEvaluationData))uiRefresh(false)};
...
$("openYahoo").addEventListener("click",()=>chrome.tabs.create({url:"https://football.fantasysports.yahoo.com/f1/mock_lobby"}));
